...
export const ChatInterface: React.FC = () => {
  ...
  return (
        <div className="flex h-screen bg-white font-sans text-slate-900 selection:bg-slate-200 selection:text-slate-900">
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col h-full relative max-w-5xl mx-auto w-full shadow-2xl shadow-slate-200/50 bg-white border-x border-slate-50">
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-20">
          <div className="flex items-center space-x-3 group cursor-pointer">
            <div className="relative">
                            <div className="absolute inset-0 bg-slate-300 blur-lg opacity-20 group-hover:opacity-40 transition-opacity rounded-full"></div>
                            <div className="p-2 bg-slate-900 rounded-xl shadow-lg relative z-10 group-hover:scale-105 transition-transform">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium flex items-center">
                                <span className="w-1.5 h-1.5 bg-slate-400 rounded-full mr-1.5 animate-pulse"></span>
                Contango Agent
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-10 scroll-smooth custom-scrollbar">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 animate-in fade-in zoom-in-95 duration-700">
                            <div className="w-24 h-24 bg-gradient-to-tr from-slate-50 to-slate-100 rounded-3xl flex items-center justify-center mb-8 shadow-inner ring-1 ring-slate-100 overflow-hidden relative">
                                <div className="absolute inset-0 bg-grid-slate-200/50 [mask-image:linear-gradient(0deg,white,rgba(255,255,255,0.6))]"></div>
                                <Sparkles className="w-10 h-10 text-slate-700 relative z-10" />
              </div>
              ...
              <p className="text-slate-500 max-w-md text-lg leading-relaxed mb-10">
                I can analyze e-commerce sales, customers, returns, inventory,
                and marketing data to help you find real insights.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl w-full">
                {[
                  {
                    t: "Revenue Trend",
                    q: "Show the monthly revenue trend for 2023",
                  },
                  {
                    t: "Return Hotspots",
                    q: "Which product categories have the highest return rate?",
                  },
                  {
                    t: "Payment Habits",
                    q: "What's the most common payment method, and what percentage of transactions use it?",
                  },
                  {
                    t: "Just Curious",
                  ...
                  <button
                    ...
                    className="group p-4 bg-white border border-slate-200 rounded-2xl text-left hover:border-slate-300 hover:shadow-lg hover:shadow-slate-200/50 transition-all duration-300 relative overflow-hidden"
                  >
          ...
          {messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                {msg.role === "assistant" && (
                                    <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center flex-shrink-0 shadow-lg shadow-slate-900/20 mt-1 ring-2 ring-white z-10">
        ...
        <div className="p-6 bg-white/80 backdrop-blur-lg border-t border-slate-100 flex justify-center">
          <form
            ...
            className="max-w-3xl w-full relative flex items-center transition-all"
          >
                        <div className="absolute inset-0 bg-gradient-to-r from-slate-100 to-slate-200 rounded-2xl blur-xl opacity-20 pointer-events-none"></div>
                        <div className="relative w-full flex items-center bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/20 focus-within:ring-2 focus-within:ring-slate-300/40 focus-within:border-slate-300 transition-all overflow-hidden">
              <input
                type="text"
                value={input}
              ...
              <div className="pr-2 flex items-center">
                {isLoading ? (
                  ...
                  <button
                    type="submit"
                    disabled={!input.trim()}
                                        className="p-3 mr-1 bg-slate-900 text-white rounded-xl hover:shadow-lg hover:shadow-slate-900/30 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95 duration-200"
                  >
